...
| `claude-mem` | `13.24.1` nos dois hosts; Bruno Vascular excluído. **Ligado só no Claude Code** — no Codex está `enabled = false` e o fallback `UserPromptSubmit` do `config.toml` está comentado desde 2026-09-07 (`T-074`), para o piloto do `T-078` medir uma camada só | **Rede de segurança**, não fonte da verdade. A instalação está coerente, mas a captura ainda está degradada: canários Codex gravaram sessão/resumo sem prompt ou observação, e o Claude temporário ficou sem `memory_session_id`. Verifique pelo `claude_mem_status.py`, nunca só por health; a wiki vence em conflito |
...
| `ai-memory` | binário `2.0.0` em `~/Applications/ai-memory/`, serviço `launchd` persistente, loopback `127.0.0.1:49374`, data-dir em `~/Library/Application Support/ai-memory` | **Em avaliação (`T-078`)** — segunda camada de captura automática, ao lado do claude-mem, para comparação. `llm: disabled, embedding: disabled`: nada sai da máquina. Instalado **global, capturando em todos os projetos** por decisão do dono, contra a recomendação do planner e do revisor. **Claude e Codex capturam** — validado em 2026-09-05; o `hooks.json` é do app Terminals, que o reescreve, então os hooks vivem em `~/.codex/config.toml`, nos seis eventos. Desde 2026-09-07 é a **única** camada de captura do Codex (`T-074`) |
