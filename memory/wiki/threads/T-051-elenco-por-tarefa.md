...
## Nota herdada do card `T-051` (migrada em 2026-09-01)
...
**Título na época:** Reestruturar o elenco por natureza da tarefa e aposentar o Kimi

🟡 **pedido do dono em 2026-09-01**: *"retirar o uso do Kimi, vou cancelar a assinatura, não acho que esteja trazendo tanto benefício"* + *"vi essa configuração e achei interessante uma maneira da gente distribuir melhor essa arquitetura de agentes e multi-agentes… talvez uma adaptação para todo tipo de desenvolvimento, porque nem sempre tem front-end, nem sempre está no back-end"*. **Dois pedidos, um card só** porque batem nos mesmos arquivos (`elenco.md`, `revisar.md`, `instalar.md`, `SKILL.md`, `stack.md`, `lint-coerencia.py`) — separá-los é retrabalho garantido, o padrão que já queimou em `T-023`/`T-025`/`T-020`. **(A) Aposentar o Kimi:** 10 arquivos do produto (`instalar.md` 25 ocorrências · `revisar.md` 19 · `elenco.md` 13 · `stack.md` 10 · `SKILL.md` 5 · `lint-coerencia.py` 5 · `plan-next.md` 2 · `init.md`, `implement-next.md`, `ajuda.md` 1) + `README.md`/`CLAUDE.md`/`AGENTS.md` + fora do produto o agente global `~/.claude/agents/kimi-revisor.md`, a regra do `~/.claude/CLAUDE.md` e as cópias em `~/.agents/skills/orq/` e `~/.kimi-code/agents/` (essas quatro são **recomendação ao dono**, nunca edição nossa). O painel cai de 3 vendors para 2 — **a diversidade cross-vendor sobrevive** (Anthropic × OpenAI), que é o que a regra do painel exige. **(B) Elenco por natureza da tarefa:** hoje os papéis são de processo (`planner`/`implementer`/`reviewer`/`docs`/`scout`); a referência que ele trouxe distribui por **tipo de trabalho** (planejamento · arquitetura técnica · frontend · UI/UX difícil · backend · código normal · UI simples · mudança pequena e clara · tarefa mecânica) — e ele quer isso **generalizado**, sem presumir que todo projeto tem front e back. Precisa continuar resolvendo por host: **sessão no Claude** e **sessão no GPT/Codex** dão times diferentes. **Nota factual:** `gpt-5.6-sol` e `gpt-5.6-terra` já estão no `_elenco.md` e o `~/.codex/config.toml` roda `gpt-5.6-sol@high`; **`gpt-5.6-luna` ainda não aparece em lugar nenhum — o planner tem que confirmar que existe antes de desenhar em cima dele**. Planejado com `fable` em 2026-09-01; plano em `memory/wiki/threads/T-051-elenco-por-tarefa.md`. ✅ **AS 4 DECISÕES ESTÃO FECHADAS (dono, 2026-09-01) — não repergunte nenhuma.** **(1) O dono corrigiu a leitura da referência, e a correção é o coração do card:** a configuração tem **dois eixos**, não um — o **domínio** escolhe o vendor (interface/experiência → Anthropic: `Frontend→Opus 5`, `Hard UI/UX→Fable 5`, `Simple UI→Sonnet 5`; sistema/lógica/dados → OpenAI: `Technical architecture→Sol`, `Backend→Sol`, `Normal coding→Terra`, `Small changes→Luna`) e a **dificuldade** escolhe o degrau dentro do vendor. O primeiro plano colapsou tudo em dificuldade e perdeu o eixo de domínio; o dono pegou o erro no gate (*"ou você interpretou errado ou você viu errado?"*). Generalização: o eixo **não é frontend/backend**, é **interface vs sistema** — CLI é sistema, brand book é interface; projeto sem UI só não usa a linha de cima. **Como o eixo sobrevive à restrição de escrita:** *"domínio decide quem pensa, host decide quem escreve"* — planner/revisor/scout são read-only e podem ser de qualquer vendor (`codex exec -s read-only` e `run-opus-reviewer.py` já comprovados), então é aí que a aptidão por domínio entra; o **implementer fica no vendor do host, sempre**, modulado só pela dificuldade. Escrita cross-vendor segue **fora do desenho** (`T-021`, um writer por worktree). **(2) Revisor único, sempre do vendor OPOSTO ao host** — regra dada pelo dono: *"sempre tem que ser com um revisor de uma LLM diferente; se eu estiver no Claude, o revisor tem que ser do GPT, e se eu estiver no GPT, do Claude"*. Isso **descarta a diagonal OpenAI** que o planner havia recomendado e **remove o revisor interno**. ⚠️ Quando o eixo de domínio e a regra do revisor divergirem (tarefa de interface no host Claude), **a independência ganha**: o revisor é oposto ao host por definição, e o domínio modula o *planner*, nunca o vendor do revisor — a razão de existir do revisor é independência, não aptidão. **Consequência assumida:** o painel vira parecer único, *"confirmado por 2+"* **deixa de existir** e todo achado é solitário por construção. **(3)** `gpt-5.6-luna` entra como implementer·leve no Codex, **condicionado a 1 smoke** — existe em `~/.codex/models_cache.json` ("Fast and affordable agentic coding model", sucessor do GPT-5.4 Mini) e em `config.toml:362`, mas **nunca foi invocado nesta máquina** e o catálogo não expõe os efforts; até o smoke passar, a faixa leve cai em `terra@xhigh`. **(4)** A limpeza dos artefatos Kimi **na máquina do dono** (`~/.claude/agents/kimi-revisor.md`, a seção de delegação do `~/.claude/CLAUDE.md`, `~/.agents/skills/orq/`, `~/.kimi-code/`) só **depois** do release validado e do cancelamento efetivo da assinatura — reversível; o card entrega o checklist, ele aplica. **(5) SEM exceções à regra do revisor oposto** — decidido em 2026-09-01 **contra a recomendação do planner**, que propunha três (dado sensível · titular fora do ar · `--rapido`). O dono leu o custo e escolheu a regra pura. Consequências **de execução obrigatória**, escritas na seção 5 da thread: diff com dado sensível → **não há revisor nenhum** (LGPD impede o vendor oposto, e não existe substituto interno), o Manager audita e **declara a ausência** — nunca spawnar revisor do mesmo vendor para tapar o buraco; titular indisponível → **REVISÃO DEGRADADA** e o card não avança sozinho; `--rapido` vira briefing enxuto para o mesmo titular externo, e a linha *"Pequeno → implemente + revisor interno"* de `SKILL.md:29` **é reescrita por este card**. ⚠️ **Colisão de numeração:** o plano propõe `0.24.0` porque o board reserva `0.23.0` ao `T-042` — decisão operacional do Manager no release: quem sair primeiro leva o menor. **Implementação autorizada em 2026-09-01** (host Claude, `opus` por exceção justificada). ✅ **IMPLEMENTADO E APROVADO — aguarda SUA ordem de release.** Branch `feat/t051-elenco-por-tarefa`, 18 arquivos, `0.24.0` nos quatro lugares, gates verdes conferidos pelo Manager (não só relatados). **7 rodadas de review externo (GPT, vendor oposto): 5→8→4→5→2→1→0 APROVADO**, 25 bloqueadores corrigidos. Os quatro mais caros: (1) **duas fontes de verdade no elenco** — `## Papéis` se dizia ativa e era o que os perfis reescreviam, enquanto os consumidores liam `## Times por host`: `perfil economia` no Codex era **inócuo**; a tabela foi eliminada; (2) a correção do piso de faixa **apagou o piso de Alto risco** — schema/segurança cairia no implementer mais fraco; (3) o exemplo canônico da SKILL (*"implemento com o Sonnet e mando revisar pelo GPT"*) era **duplamente inválido no Codex**; virou nomeação de papéis resolvidos; (4) o comando aceitava registrar `fable` como planner no Codex, mas o runner invoca `--model opus` **fixo** — elenco declarava um modelo e executava outro, calado. Smoke do `gpt-5.6-luna` **passou** (`LUNA_OK`), liberando `implementer·leve` do Codex **sem effort declarado** — o smoke provou que responde, não quais efforts aceita nem como se comporta em `workspace-write`. Cobertura mecânica subiu de 2 para **5 famílias** de defeito, com 9 regressões negativas provadas; **22 dos 25 bloqueadores vieram de coerência entre superfícies**, que segue manual. ✅ **PUBLICADO, INSTALADO E VERIFICADO (2026-09-02).** Reconciliado pelo `T-052` e publicado como **`0.25.0`** (`bd6d3fc`); cache Claude e Codex conferidos com `verify_installed_cache.py` a partir de **clone detached limpo** — `ok: installed cache matches source`, exit `0` nos dois. ⏭️ **Falta só o SEU teste comportamental** (4 frases, seção 7 da thread): *"tô com pouco crédito"* no Codex troca o time de verdade · *"melhora esse texto"* e *"otimiza o cache"* no mesmo host anunciam **planners de vendors diferentes** · *"revisa isso"* num diff com dado de paciente diz que **não haverá revisor** e não inventa um · *"dá pra instalar no Kimi?"* responde que o suporte foi aposentado. ⚠️ Conflito conhecido no merge: `memory/MEMORY.md` (uma linha, o bump). @frente-elenco
