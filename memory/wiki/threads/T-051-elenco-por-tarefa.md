...
9. **Bump `0.24.0` nos quatro lugares** (`orq/.claude-plugin/plugin.json`, README Status,
   ...
   do lint.
10. **Gates:** `claude plugin validate ./orq --strict` **e** `python3 orq/scripts/lint-coerencia.py .`
    verdes.
...
12. **Release + teste comportamental** (fecha o card): `claude plugin marketplace update orquestra`
    + `claude plugin update orq@orquestra` + restart + `diff -rq ~/.claude/plugins/cache/orquestra/
    orq/<versão>/ ./orq/` vazio + critérios da seção 7. ✅ **Smoke do luna feito em 2026-09-01**
