...
- [!] `T-013` Checkpoint concorrente entre várias janelas — **o modelo pressupõe UM Manager por projeto, e o dono usa N janelas.** Hoje não há detecção, identidade nem merge: `KANBAN.md` e as páginas de tópico são **reescritas** (last-write-wins → movimento de card some), `fixes-history.md` é append por ler-modificar-escrever (duas janelas simultâneas perdem uma entrada), e nada registra **quem** escreveu. A perda é **silenciosa** — mesma classe do bug do board. **Desenho proposto (não aprovado):** (a) *detecção* — o checkpoint compara mtime/hash do que leu com o estado em disco e, se mudou, faz merge consciente em vez de sobrescrever; (b) *segregação por natureza* — trabalho em curso vai para `threads/<janela>.md`, que é naturalmente livre de conflito, e só o board é disputado; (c) *claim de card* — a janela marca o card como seu antes de trabalhar, e quem chega depois escolhe outro (lock cooperativo no nível do card, não do arquivo); (d) *append real* no log, sempre relendo o fim antes de escrever. **Recusado de saída:** lock global (mata o paralelismo que o dono quer) e merge por git (markdown de prosa conflita mal, e ele não commita a cada checkpoint). **Decisão do dono pendente:** quantas janelas de fato usa em paralelo, e se topa o claim explícito de card.
