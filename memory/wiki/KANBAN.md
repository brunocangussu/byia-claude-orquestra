...
- [ ] `T-003` Piloto end-to-end pelos **comandos** — 🔴 **parcialmente cumprido em 2026-07-26, mas pelo caminho errado.** Houve piloto real (o T-009 passou por implementação + painel + 8 achados aplicados) e ele já rendeu o relatório de atrito: 2 causas raiz no painel (`T-010`), o falso positivo do lint em `memory/` (`T-008`) e a confusão plugin-vs-CLI do Codex. **Mas tudo isso foi feito pelo Manager na mão, não invocando `/orq:plan-next` e `/orq:implement-next`.** O que falta validar é justamente o que não foi testado: **os comandos funcionam quando invocados de verdade?** Rodar um card do backlog pelo fluxo formal, sem atalho, e registrar onde travar.
