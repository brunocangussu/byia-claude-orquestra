...
⚠️ **Mexeu em `orq/`? O primeiro passo é o bump — antes dos gates, no mesmo commit.** A versão
vive em **quatro** lugares e os quatro andam juntos: `orq/.claude-plugin/plugin.json` ·
`.claude-plugin/marketplace.json` · a seção **Status** deste README · `memory/MEMORY.md`.

Isto é passo do procedimento, não recomendação: **o cache é indexado por versão**, então publicar
sem bump reaproveita a chave e o cache conserva os bytes antigos — e **nenhum gate acusa**. O teste
de coordenação só compara os quatro entre si, e eles continuam iguais; o lint não confere um cache
que não existe naquela máquina ou no CI. É o `T-017` do board, que já custou sete releases com o
`.claude-plugin/marketplace.json` parado em `0.4.0`.

Ao editar, **os três gates automatizados, nesta ordem e todos obrigatórios**:
...
A suíte cobre verificador de instalação, guardião de contexto, runner Opus e os dois auditores — e é
**descoberta, nunca enumerada**, para não envelhecer quando um módulo novo entrar. Só depois vêm
`/plugin marketplace update orquestra` + `/plugin update orq@orquestra`. Para iterar numa
...
`0.27.10` — board · time · dois loops · memória-wiki · interface natural · modo noturno (planejamento)
